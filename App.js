...
function App() {
    ...
    const [teaserTarget, setTeaserTarget] = useState(null);
    ...
    const plan = profile && profile.plan ? profile.plan : 'Starter';
    ...
    useEffect(() => {
        const handlePopState = () => {
            ...
            if (path && path !== view) {
                ...
                const validViews = ['dashboard', 'profile', 'full-review', 'career-intelligence', 'applications', 'rebuild-standalone', 'resume-editor', 'pricing', 'settings', 'billing', 'faq', 'contact', 'admin'];
    ...
    const dispatchJob = useCallback(async (type, payload) => {
        ...
        if (type === 'REBUILD' && !finalPayload.resume_id) {
            finalPayload.resume_id = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2);
        }
        ...
        (async () => {
            try {
                let result;
                if (!user)
                    throw new Error("Must be logged in to execute jobs");
                ...
                if (type === 'ANALYSIS') {
                    ...
                    const { data, error } = await supabase.functions.invoke('generate-rebuild', { body: commonBody });
                    if (error)
                        throw error;
                    ...
                    if (remote.status === 'completed') {
                        ...
                        try {
                            if (job.type === 'ANALYSIS') {
                                ...
                                if (data)
                                    fetchedResult = { ...data.results_json, analysisId: data.id };
                            }
                                ...
                                if (rid) {
                                    ...
                                    if (data)
                                        fetchedResult = { newResume: data.data };
                                }
                            }
                                ...
                                if (data)
                                    fetchedResult = data.results_json;
                            }
                        }
                        catch (err) {
    ...
    async function fetchUserData(authUser) {
        ...
        if (resumes) {
            const mapped = resumes.map(function (r) {
                return {
                    id: r.id,
                    name: r.name,
                    versions: r.resume_versions.map(function (v) {
                        return {
                            versionId: v.id,
                            type: v.version_type,
    ...
    const handleSaveToProfile = async (version, label) => {
        if (!profile || !user)
            return;
    ...
    const handleSetView = function (targetView) {
    ...
    return (_jsxs("div", { className: "min-h-screen bg-[#0F1117] flex flex-col antialiased", children: [showNav && (_jsx(Header, { currentView: teaserTarget || view, setView: handleSetView, plan: plan, onNewResume: () => handleSetView('resume-editor') })), hasActiveJob && showNav && (_jsxs("div", { className: "fixed bottom-6 right-6 z-[200] bg-[#16161E] border border-blue-500/30 px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-4 animate-in slide-in-from-bottom-4 duration-500", children: [_jsx("div", { className: "w-2 h-2 rounded-full bg-blue-500 animate-pulse" }), _jsx("span", { className: "text-[10px] font-black text-white uppercase tracking-widest", children: "Background Engine Active" }), _jsx(Loader2, { size: 14, className: "text-blue-500 animate-spin" })] })), _jsx("main", { className: "flex-1 flex flex-col " + (showNav ? 'pt-20' : ''), children: _jsx("div", { className: "flex-1 overflow-y-auto", children: teaserTarget ? (_jsxs("div", { className: "animate-in fade-in duration-500", children: [_jsx(FeatureTeaser, { targetView: teaserTarget, onUpgrade: function () { setTeaserTarget(null); setView('pricing'); } }), _jsx("div", { className: "bg-[#0F1117] pb-24", children: _jsx(Pricing, { setPlan: async function (p) {
                                        if (user) {
                                    ...
                                    }, setView: handleSetView, currentPlan: plan }) })] })) : (_jsxs(_Fragment, { children: [view === 'landing' && _jsx(LandingPage, { onGetStarted: function () { handleSetView(user ? 'dashboard' : 'auth'); }, onViewPlans: function () { handleSetView('pricing'); } }), view === 'auth' && _jsx(AuthView, { onSuccess: function () { handleSetView('dashboard'); } }), view === 'auth-bridge' && _jsx(AuthBridge, {}), view === 'dashboard' && (_jsx(DashboardView, { currentAnalysis: currentAnalysis, plan: plan, onNavigate: handleSetView })), view === 'applications' && _jsx(ApplicationExecutionView, { user: profile }), view === 'ai-review' && _jsx(AIReviewView, { plan: plan, onResult: (r) => {
                                    setAnalysisHistory(prev => ({ ...prev, [r.analysisId]: r }));
                                    setActiveAnalysisId(r.analysisId);
                                    handleSetView('full-review');
                                }, onUpload: (t) => { }, pendingResumeText: pendingResumeText, onUpgrade: function () { handleSetView('pricing'); }, onStartScratch: () => handleSetView('resume-editor'), activeJobs: jobs, dispatchJob: dispatchJob }), view === 'full-review' && _jsx(FullReviewView, { result: currentAnalysis, plan: plan, onUpgrade: function () { handleSetView('pricing'); }, onRebuildRequest: (id) => handleSetView('rebuild-standalone'), setView: handleSetView }), view === 'career-intelligence' && _jsx(CareerIntelligenceView, { analysisResult: currentAnalysis, resumeText: currentAnalysis && currentAnalysis.resumeText ? currentAnalysis.resumeText : '', plan: plan, setView: handleSetView, activeJobs: jobs, dispatchJob: dispatchJob }), view === 'rebuild-standalone' && _jsx(RebuildStandaloneView, { plan: plan, credits: profile && profile.credits ? profile.credits : 0, setCredits: async function (c) { }, onRebuildSuccess: async function (rebuilt, vid, label, gid) {
                                    if (user)
                                ...
                                }, onUpgrade: function () { handleSetView('pricing'); }, history: resumeHistory, activeJobs: jobs, dispatchJob: dispatchJob }), view === 'profile' && _jsx(ProfileView, {}), view === 'history' && _jsx(ResumeHistoryView, { history: resumeHistory, analysisHistory: analysisHistory, onEdit: function (gid, vid) {
                                    setEditingResumeId(gid);
                                    setEditingVersionId(vid);
                                    handleSetView('resume-editor');
                                    ...
                                    setEditingVersionId(vid);
                                    handleSetView('resume-editor');
                                }, onStartNew: function () { handleSetView('ai-review'); }, onSaveToProfile: handleSaveToProfile }), view === 'preview' && _jsx(ExecutionPreviewView, { onNavigate: (view) => handleSetView(view) }), view === 'resume-editor' && _jsx(ResumeBuilder, { plan: plan, groupId: editingResumeId, versionId: editingVersionId, history: resumeHistory, onBack: function () { handleSetView('dashboard'); } }), view === 'pricing' && _jsx(Pricing, { setPlan: async function (p) { }, setView: handleSetView, currentPlan: plan }), view === 'settings' && _jsx(AccountSettings, { plan: plan, profile: profile }), view === 'billing' && _jsx(Billing, { plan: plan, setView: handleSetView }), view === 'faq' && _jsx(FAQ, { setView: handleSetView }), view === 'contact' && _jsx(Contact, {}), view === 'admin' && _jsx(AdminIntelligence, {})] })) }) })] }));
}
export default App;

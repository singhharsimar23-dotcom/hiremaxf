...
function App() {
    const { state, context, user, error, connectAuth, executeAutoFill, stopExecution } = useExtensionState();
    ...
    return (
        <div className="w-[360px] h-[500px] bg-background text-text-primary flex flex-col font-sans overflow-hidden">
            ...
            <div className="h-12 border-t border-border bg-card flex items-center justify-between px-4 text-xs text-text-secondary">
                <div className="flex items-center space-x-2">
                    <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]"></div>
                    <span>v1.0.2 &bull; {user ? user.email : 'Unknown User'}</span>
                </div>

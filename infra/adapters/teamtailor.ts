/**
 * infra/adapters/teamtailor.ts
 * TEAMTAILOR ADAPTER — HARDENED V5.2 (Cloudflare Optimized)
 */

import type { Env, ParsedJob } from '../workers/types/job.ts';
import type { ConnectorAdapter } from './interface.ts';
import { fetchTeamtailorJobs } from '../connectors/teamtailor.ts';
import { parseTeamtailorJob } from '../../core/ingestion-engine/parsers/teamtailor.ts';
...
export const TeamtailorAdapter: ConnectorAdapter = {
  async fetchBatch(env: Env, slug: string, offset: number, limit: number): Promise<any[]> {
    let rawBatch: any[] = [];
    try {
      rawBatch = await timeout(
        fetchTeamtailorJobs(env, slug, offset, limit),
        FETCH_TIMEOUT_MS
      );
    } catch (e: any) {
      console.error(`[teamtailor:${slug}] fetch failed: ${e.message}`);
      throw e;
    }
  ...
  async parse(raw: any, label: string): Promise<ParsedJob> {
    if (!isValidRawJob(raw)) throw new Error('INVALID_RAW_JOB: Teamtailor payload sanity check failed');
    try {
      const parsed = await parseTeamtailorJob(raw, label);
      parsed.source = 'teamtailor';
      return parsed;
    } catch (e: any) {
      throw new Error(`PARSE_FAILURE: [teamtailor] ${e.message}`);
    }
  },

  async healthCheck(env: Env, slug: string): Promise<boolean> {
    try {
      await timeout(fetchTeamtailorJobs(env, slug, 0, 1), 5000);
      return true;
    } catch {
      return false;
    }

/**
 * infra/adapters/personio.ts
 * PERSONIO ADAPTER — HARDENED V5.2 (Cloudflare Optimized)
 */

import type { Env, ParsedJob } from '../workers/types/job.ts';
import type { ConnectorAdapter } from './interface.ts';
import { fetchPersonioJobs } from '../connectors/personio.ts';
import { parsePersonioJob } from '../../core/ingestion-engine/parsers/personio.ts';
...
export const PersonioAdapter: ConnectorAdapter = {
  async fetchBatch(env: Env, slug: string, offset: number, limit: number): Promise<any[]> {
    let rawBatch: any[] = [];
    try {
      rawBatch = await timeout(
        fetchPersonioJobs(env, slug, offset, limit),
        FETCH_TIMEOUT_MS
      );
    } catch (e: any) {
      console.error(`[personio:${slug}] fetch failed: ${e.message}`);
      throw e;
    }
  ...
  async parse(raw: any, label: string): Promise<ParsedJob> {
    if (!isValidRawJob(raw)) throw new Error('INVALID_RAW_JOB: Personio payload sanity check failed');
    try {
      const parsed = await parsePersonioJob(raw, label);
      parsed.source = 'personio';
      return parsed;
    } catch (e: any) {
      throw new Error(`PARSE_FAILURE: [personio] ${e.message}`);
    }
  },

  async healthCheck(env: Env, slug: string): Promise<boolean> {
    try {
      await timeout(fetchPersonioJobs(env, slug, 0, 1), 5000);
      return true;
    } catch {
      return false;
    }

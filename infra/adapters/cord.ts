/**
 * infra/adapters/cord.ts
 * CORD ADAPTER — HARDENED V5.2 (Cloudflare Optimized)
 */

import type { Env, ParsedJob } from '../workers/types/job.ts';
import type { ConnectorAdapter } from './interface.ts';
import { fetchCordJobs } from '../connectors/cord.ts';
import { parseCordJob } from '../../core/ingestion-engine/parsers/cord.ts';
...
export const CordAdapter: ConnectorAdapter = {
  async fetchBatch(env: Env, slug: string, offset: number, limit: number): Promise<any[]> {
    let rawBatch: any[] = [];
    try {
      rawBatch = await timeout(
        fetchCordJobs(env, slug, offset, limit),
        FETCH_TIMEOUT_MS
      );
    } catch (e: any) {
      console.error(`[cord:${slug}] fetch failed: ${e.message}`);
      throw e;
    }
  ...
  async parse(raw: any, label: string): Promise<ParsedJob> {
    if (!isValidRawJob(raw)) throw new Error('INVALID_RAW_JOB: Cord payload sanity check failed');
    try {
      const parsed = await parseCordJob(raw, label);
      parsed.source = 'cord';
      return parsed;
    } catch (e: any) {
      throw new Error(`PARSE_FAILURE: [cord] ${e.message}`);
    }
  },

  async healthCheck(env: Env, slug: string): Promise<boolean> {
    try {
      await timeout(fetchCordJobs(env, slug, 0, 1), 5000);
      return true;
    } catch {
      return false;
    }

...
import { TruthLab } from './profile/TruthLab';

import * as pdfjsLib from 'pdfjs-dist';


// Use CDN for worker to avoid Vite public folder resolution issues
...
interface ProfileSnapshot {
    ...
    eligibility_signals?: {
        ...
        salary_expectation_hint?: string;
    };
    created_at: string;
}
...
export const ProfileView: React.FC = () => {
    ...
    const [viewMode, setViewMode] = useState<'standard' | 'health' | 'truth'>('standard');
    ...
    const updateEligibilitySignals = async (newSignals: any) => {
        try {
            ...
            const { error } = await supabase
                ...
                .update({ eligibility_signals: newSignals })
                .eq('id', snapshot.id);

            if (error) throw error;
            setSnapshot(prev => prev ? { ...prev, eligibility_signals: newSignals } : null);
            setSuccessMessage("Eligibility signals refined.");
        } catch (e: any) {
    ...
    const executeIngestion = async (params: {
        ...
        try {
            ...
            console.log('INGESTION', `Ingestion v2.5 Session Initialized: ${sessionToken}`);
    ...
    const handleLinkedInPDFUpload = async (file: File) => {
        ...
        try {
            ...
            if (file.type === 'application/pdf') {
                ...
                throw new Error("Only PDF files are supported for LinkedIn Truth Ingestion.");
            }

            // Relaxed heuristic: Just check if it has LinkedIn related keywords to prevent major mistakes
            // But don't be too strict as different exports/languages may vary.
            const profileKeywords = ["LinkedIn", "Experience", "Education", "Contact", "Profile"];
            const hasKeywords = profileKeywords.some(k => fullText.toLowerCase().includes(k.toLowerCase()));
            
            if (!hasKeywords) {
                console.warn("PDF may not be a LinkedIn profile, but proceeding anyway with v2 worker.");
            }

            setLoadingMessage("Initiating Forensic Analysis...");
            await executeIngestion({
                source: 'LINKEDIN',
                type: 'FILE',
                action: 'INGEST',
                payload: {
                    text: fullText,
                    source_type: 'PDF_FORENSICS',
                    file_name: file.name
                }
            });

            setSuccessMessage("LinkedIn Truth Ingestion started!");
        } catch (err: any) {
            setErrorMessage(`LinkedIn Parsing Failed: ${err.message}`);
        } finally {
    ...
    const filteredLogs = [
        ...Object.entries(commandStatuses).map(([id, status]) => ({
            id: `cmd-${id}`,
            event_type: 'INGESTION',
            message: `Processing source synchronization... (${status.toUpperCase()})`,
        ...
        <div className="max-w-[1400px] mx-auto px-10 py-12 animate-in fade-in duration-700 bg-[#0A0B10] min-h-screen text-slate-200">
            ...
            <div className="flex gap-4 mb-10">
                ...
                <button
                    ...
                    className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${viewMode === 'truth' ? 'bg-emerald-600 text-white shadow-xl shadow-emerald-600/20' : 'bg-slate-900 text-slate-500 border border-slate-800 hover:text-white'}`}
                >
                    Truth Lab
                </button>
            </div>
            ...
            {viewMode === 'truth' ? (
                <TruthLab evidence={manualEvidence} onActionComplete={() => fetchProfileData()} />
                ...
                <div className="space-y-12">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 h-16 opacity-10 pointer-events-none">
                                    {strengthHistory && strengthHistory.length > 1 && (
                                        <svg className="w-full h-full" viewBox={`0 0 ${strengthHistory.length - 1} 100`} preserveAspectRatio="none">
                                            <path d={`M 0 ${100 - (strengthHistory[0]?.total_score || 0)} ${strengthHistory.filter(s => s).map((s, i) => `L ${i} ${100 - (s.total_score || 0)}`).join(' ')}`} fill="none" stroke="#3b82f6" strokeWidth="4" />
                                        </svg>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                        <div className="lg:col-span-8 space-y-10">
                            ...
                            <div className="bg-[#12141C] border border-[#23262F] rounded-[2.5rem] p-10 shadow-2xl">
                                <div className="flex items-center gap-4 mb-8">
                                    <div className="p-3 bg-indigo-500/10 rounded-2xl">
                                        <ShieldCheck className="text-indigo-500" size={24} />
                                    </div>
                                    <div>
                                        <h2 className="text-2xl font-bold text-white tracking-tight">Eligibility Forensics</h2>
                                        <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Automated Signal Detection</p>
                                    </div>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="bg-[#0A0B10] border border-[#23262F] p-6 rounded-[2rem] hover:border-indigo-500/30 transition-all">
                                        <div className="flex justify-between items-center mb-4">
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Work Authorization</span>
                                            <Globe size={14} className="text-slate-600" />
                                        </div>
                                        <select 
                                            value={snapshot?.eligibility_signals?.work_authorization || 'UNKNOWN'}
                                            onChange={(e) => updateEligibilitySignals({ ...snapshot?.eligibility_signals, work_authorization: e.target.value })}
                                            className="w-full bg-[#1A1D26] border border-[#2D313D] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors appearance-none cursor-pointer"
                                        >
                                            <option value="UNKNOWN">DETECTING...</option>
                                            <option value="CITIZEN">US CITIZEN / GC</option>
                                            <option value="H1B_HOLDER">H1-B HOLDER</option>
                                    ...
                                    <div className="bg-[#0A0B10] border border-[#23262F] p-6 rounded-[2rem] hover:border-indigo-500/30 transition-all">
                                        ...
                                        <select 
                                            value={snapshot?.eligibility_signals?.security_clearance || 'NONE'}
                                            onChange={(e) => updateEligibilitySignals({ ...snapshot?.eligibility_signals, security_clearance: e.target.value })}
                                            className="w-full bg-[#1A1D26] border border-[#2D313D] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors appearance-none cursor-pointer"
                                        >
                                            <option value="NONE">NO CLEARANCE</option>
                                            <option value="SECRET">SECRET</option>
                                            <option value="TS_SCI">TS/SCI</option>
                                            <option value="POLYGRAPH">FULL SCOPE POLY</option>
                                    ...
                                    <div className="bg-[#0A0B10] border border-[#23262F] p-6 rounded-[2rem] hover:border-indigo-500/30 transition-all">
                                        <div className="flex justify-between items-center mb-4">
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Location Preference</span>
                                            <Globe size={14} className="text-slate-600" />
                                        </div>
                                        <input 
                                            type="text"
                                            value={snapshot?.eligibility_signals?.location_preference || ''}
                                            onChange={(e) => setSnapshot(prev => prev ? { ...prev, eligibility_signals: { ...prev.eligibility_signals, location_preference: e.target.value } } : null)}
                                            onBlur={(e) => updateEligibilitySignals({ ...snapshot?.eligibility_signals, location_preference: e.target.value })}
                                            placeholder="City, State or 'Remote'"
                                            className="w-full bg-[#1A1D26] border border-[#2D313D] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors"
                                        />
                                    </div>

                                    <div className="bg-[#0A0B10] border border-[#23262F] p-6 rounded-[2rem] hover:border-indigo-500/30 transition-all flex flex-col justify-center">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1">Open To Relocation</span>
                                                <p className="text-[10px] text-slate-600 font-bold uppercase">Enable cross-regional matching</p>
                                            </div>
                                            <button 
                                                onClick={() => updateEligibilitySignals({ ...snapshot?.eligibility_signals, relocation_intent: !snapshot?.eligibility_signals?.relocation_intent })}
                                                className={`w-12 h-6 rounded-full relative transition-all ${snapshot?.eligibility_signals?.relocation_intent ? 'bg-indigo-600' : 'bg-slate-800'}`}
                                            >
                                                <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${snapshot?.eligibility_signals?.relocation_intent ? 'left-7' : 'left-1'}`} />
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-[#12141C] border border-[#23262F] rounded-[2.5rem] p-8 flex flex-col group hover:border-blue-500/30 transition-all">
                                    ...
                                    <div className="mt-auto pt-6 border-t border-[#23262F]">
                                        {Object.values(commandStatuses).some(s => s === 'processing') && integrityLogs.some(l => l.source === 'LINKEDIN_WORKER' && l.message.toLowerCase().includes('started')) ? (
                                            ...
                                            <button onClick={() => linkedinFileInputRef.current?.click()} className="w-full bg-slate-900 text-slate-300 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-white hover:text-black transition-all border border-slate-700">Upload PDF Profile</button>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="bg-[#12141C] border border-[#23262F] rounded-[2.5rem] p-8 flex flex-col group hover:border-blue-500/30 transition-all">
                                    ...
                                    <div className="mt-auto pt-6 border-t border-[#23262F]">
                                        {Object.values(commandStatuses).some(s => s === 'processing') && integrityLogs.some(l => l.source === 'GITHUB_WORKER' && l.message.toLowerCase().includes('started')) ? (
                                            ...
                                            <button onClick={() => triggerResync('github')} className="w-full bg-blue-600/30 text-blue-300 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest cursor-not-allowed">Source Locked</button>
                                        ) : (
                            ...
                            <div className="bg-[#12141C] border border-[#23262F] rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
                                ...
                                <div className="space-y-6 max-h-[350px] overflow-y-auto pr-4 custom-scrollbar">
                                    {filteredLogs && filteredLogs.length > 0 ? filteredLogs.map((log) => {
                                        ...
                                        return (
                                            <div key={log.id || Math.random()} className={`pb-4 border-b border-[#23262F] last:border-0 hover:border-blue-500/20 transition-all ${(log as any).isProcessing ? 'animate-pulse' : ''}`}>
                                                <div className="flex justify-between items-start mb-2">
                                                    <span className={`text-[8px] font-black px-2 py-0.5 rounded uppercase ${log.event_type === 'ERROR' ? 'bg-red-500 text-white' : (log as any).isProcessing ? 'bg-blue-500 text-white' : 'bg-blue-600/10 text-blue-500'}`}>
                                                        {(log as any).isProcessing ? 'PROCESSING' : (log.event_type || 'EVENT')}
                                                    </span>
                                                    <span className="text-[8px] text-slate-600 font-bold">{timestamp}</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    {(log as any).isProcessing && <Loader2 size={10} className="text-blue-500 animate-spin" />}
                                                    <p className={`text-[10px] leading-relaxed ${(log as any).isProcessing ? 'text-blue-400 font-bold' : 'text-slate-400'}`}>
                    ...
                    <div className="mt-16 bg-[#12141C] border border-[#23262F] rounded-[3.5rem] p-12 shadow-2xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        if (e.target.files && e.target.files[0]) {
                                            handleLinkedInPDFUpload(e.target.files[0]);
                                        }
                                    }}
                                />
                            </div>
                            ...
                            {[
                                ...
                                { id: 'linkedin', label: 'LinkedIn', icon: Linkedin, desc: 'Upload your LinkedIn "Save to PDF" for full history verification.', type: 'FILE', hint: 'LINKEDIN' },

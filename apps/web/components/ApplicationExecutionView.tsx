...
import {
    Play, Loader2, Target, Search, Building2, MapPin, ArrowRight, Check, ListOrdered, Layers, Workflow, Radio, FileText, RefreshCw, Briefcase, Box, ShieldCheck, ShieldX, Globe2, ShieldAlert, Cpu, Timer, BarChart, ChevronRight, AlertCircle, AlertTriangle, XCircle, Send, Clock, Calendar, Mail, Linkedin, ExternalLink, ArrowLeft, Terminal, Sparkles, UserCheck, ZapOff, CheckCircle2, History, TrendingUp
} from 'lucide-react';
import { UserPlan, UserProfile, JobOpportunity, ExecutionState, KillZoneAnalysis, ImprovementPlan, ApplicationTracking, ImprovementStep, AppView } from '../types';
...
const mapToJobOpportunity = (app: any): JobOpportunity => {
    const confidence = app.match_confidence || 0;
    return {
        id: app.id,
        ...
        confidence_tier: confidence > 0.8 ? 'HIGH' : confidence > 0.5 ? 'MEDIUM' : 'LOW',
        freshness_window: 'RECENT',
        source_ats: (() => { try { return app.source_url ? new URL(app.source_url).hostname.replace('www.', '') : 'Direct'; } catch { return 'Direct'; } })(),
        source_url: app.source_url || '',
        state: (app.status || 'TRACKED') as ExecutionState,
        analysis: confidence > 0 ? {
            inKillZone: confidence > 0.8,
            percentile: Math.round(confidence * 100),
            estimatedCallbackRate: confidence,
            confidence: confidence,
            matchBreakdown: {
                skillsMatch: confidence > 0.7 ? 'High' : confidence > 0.4 ? 'Medium' : 'Low',
                experienceMatch: confidence > 0.6,
                companyTierMatch: confidence > 0.5,
                githubScore: 0,
                minorGaps: []
            },
            competitionAnalysis: {
                estimatedApplicants: 100,
                yourRank: confidence > 0.8 ? 'Top 10' : confidence > 0.5 ? 'Top 25' : 'Top 50',
...
export const ApplicationExecutionView: React.FC<ApplicationExecutionViewProps> = ({ user, applicationId, onNavigate }) => {
    ...
    useEffect(() => {
        async function loadApplicationsData() {
            ...
            try {
                setLoading(true);
                // Direct Supabase query — bypasses the execution-engine's lossy mapping layer
    ...
    useEffect(() => {
        ...
        const loadImprovementPlan = async () => {
            ...
            if (!error) setImprovementPlan(data);
        };
        ...
        // Realtime subscription for improvement plans
        const planChannel = supabase
            .channel(`plan_${activeAppId}`)
            .on('postgres_changes', {
                event: '*',
                schema: 'public',
                ...
                if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
                    setImprovementPlan(payload.new as ImprovementPlan);
                }
            })
            .subscribe();
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 relative z-10">
                            <div className="space-y-6">
                                <div>
                                    ...
                                    {(() => {
                                        const submitted = applications.filter(a => ['SUBMITTED', 'INTERVIEW', 'UNDER_REVIEW'].includes(a.state || ''));
                                        const rate = applications.length > 0 ? Math.round((submitted.length / applications.length) * 100) : 0;
                                        return (
                                            <>
                                                <h3 className="text-4xl font-black text-white uppercase tracking-tighter">{rate}% <span className="text-blue-500">Active</span></h3>
                                                <div className="space-y-2 mt-4">
                                                    <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-slate-500">
                                                        <span>Target: 90%</span>
                                                        <span>{submitted.length} Progressing</span>
                                                    </div>
                                                    <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                                                        <div className="h-full bg-blue-600 transition-all duration-700" style={{ width: `${Math.min(rate, 100)}%` }} />
                                                    </div>
                                                </div>
                                            </>
                                        );
                            ...
                            <div className="space-y-2">
                                <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em] mb-2">Match Quality</p>
                                {(() => {
                                    const avgConfidence = applications.length > 0 ? applications.reduce((sum, a) => sum + (a.match_confidence || 0), 0) / applications.length : 0;
                                    const pctl = Math.min(99, Math.round(avgConfidence * 100));
                                    return (
                                        <>
                                            <h3 className="text-4xl font-black text-white uppercase tracking-tighter">{pctl || '--'}<span className="text-slate-500 text-sm font-bold">%</span> <span className="text-slate-500 text-xl font-bold">AVG</span></h3>
                                            <p className="text-[9px] font-bold text-slate-500 uppercase">Avg match confidence</p>
                                        </>
                                    );
                                })()}
                            </div>
                            <div className="space-y-2">
                                <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em] mb-2">Pipeline</p>
                                {(() => {
                                    const tracked = applications.filter(a => a.state === 'TRACKED').length;
                                    const active = applications.filter(a => ['SUBMITTED', 'UNDER_REVIEW', 'INTERVIEW'].includes(a.state || '')).length;
                                    const resolved = applications.filter(a => ['REJECTED', 'KILL_ZONE'].includes(a.state || '')).length;
                                    return (
                                        <>
                                            <h3 className="text-4xl font-black text-white uppercase tracking-tighter">{active} <span className="text-slate-500 text-xl font-bold">ACTIVE</span></h3>
                                            <p className="text-[9px] font-bold text-slate-500 uppercase">{tracked} tracked · {resolved} resolved</p>
                                        </>
                                    );
                                })()}
                            </div>
                            <div className="space-y-2">
                                ...
                                <h3 className="text-4xl font-black text-white uppercase tracking-tighter">{Math.max(0, 10 - applications.length)}/10 <span className="text-slate-500 text-xl font-bold">LEFT</span></h3>
                ...
                <div className="space-y-12 animate-in slide-in-from-bottom-8">
                    ...
                    <div className="bg-[#111118] border border-white/5 p-12 rounded-[3.5rem] flex flex-col md:flex-row justify-between items-start gap-12 shadow-2xl relative overflow-hidden">
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                {activeApp.job_description && (
                                    <div className="bg-white/5 border border-white/5 rounded-2xl p-6 max-h-[300px] overflow-y-auto custom-scrollbar font-mono text-[11px] leading-relaxed text-slate-400 mb-6">
                                        <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-3 border-b border-white/5 pb-2">Full Job Description</p>
                                        <div className="whitespace-pre-wrap">{activeApp.job_description}</div>
                                    </div>
                                )}
                            </div>
                        </div>
                        <div className="bg-[#161B2E] border border-white/5 p-8 rounded-[2.5rem] min-w-[320px] shadow-2xl text-center">
                                ...
                                <div className="space-y-6">
                                    <div className="flex flex-col items-center">
                                     ...
                                     <button
                                        ...
                                        className="w-full bg-amber-500/10 border border-amber-500/20 text-amber-500 font-black py-5 rounded-2xl transition-all uppercase tracking-widest text-xs">
                                        {showPlan ? 'Hide Improvement Plan' : 'See Improvement Plan'}
                                     </button>
                                </div>
                            )}
                            ...
                            {/* TRACKED / IDENTIFIED — show match confidence and job link */}
                            {['TRACKED', 'IDENTIFIED'].includes(activeApp.state || '') && (
                                <div className="space-y-6">
                                    <div className="flex flex-col items-center">
                                        {activeApp.match_confidence > 0 ? (
                                            <>
                                                <p className={`text-5xl font-black ${activeApp.match_confidence > 0.7 ? 'text-green-500' :
                                                    activeApp.match_confidence > 0.4 ? 'text-amber-500' : 'text-slate-400'
                                                    }`}>{(activeApp.match_confidence * 100).toFixed(0)}%</p>
                                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-2">Match Confidence</p>
                                            </>
                                        ) : (
                                            <>
                                                <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-slate-500 mb-2">
                                                    <Target size={28} />
                                                </div>
                                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Awaiting Analysis</p>
                                            </>
                                        )}
                                    </div>
                                    {activeApp.source_url && (
                                        <a
                                            href={activeApp.source_url}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="w-full bg-white/5 border border-white/10 hover:border-blue-500/30 text-slate-300 hover:text-white font-black py-4 rounded-2xl transition-all uppercase tracking-widest text-[10px] flex items-center justify-center gap-3"
                                        >
                                    ...
                                    <button
                                        onClick={async () => {
                                            const primaryResume = user?.resume_profiles?.find(p => p.isPrimary) || user?.resume_profiles?.[0];

                                            // Enhanced Logic: Signal Synergy Fallback
                                            if (!primaryResume) {
                                                const confirmFallback = window.confirm("No formal resume detected in your profile. Would you like to synthesize a tailored resume from your LinkedIn/GitHub signals instead?");
                                                if (confirmFallback && onNavigate) {
                                                    setSubmitting(true); // Reuse submitting as 'synthesizing' state
                                                    try {
                                                        const { CareerSynthesizer } = await import('../lib/career-synthesizer');
                                                        const synthesized = await CareerSynthesizer.synthesizeCareerContext(user!.id);

                                                        // Navigate with the rich context
                                                        onNavigate('rebuild-standalone', undefined, {
                                                            preFilled: {
                                                                text: synthesized.text,
                                                                role: activeApp.title,
                                                                track: 'STARTUP_ENG', // Default heuristic
                                                                gate: 'SAFE'
                                                            }
                                                        });
                                                    } catch (err) {
                                                        console.error("Synthesis failed:", err);
                                                        alert("Failed to synthesize profile data. Please connect LinkedIn first.");
                                                    } finally {
                                                        setSubmitting(false);
                                                    }
                                                }
                                            } else if (onNavigate) {
                                                onNavigate('rebuild-standalone');
                                            }
                                        }}
                                        disabled={submitting}
                                        className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-2xl transition-all uppercase tracking-widest text-[10px] shadow-xl flex items-center justify-center gap-3"
                                    >
                                        {submitting ? <Loader2 size={14} className="animate-spin" /> : <><Sparkles size={14} /> {user?.resume_profiles?.length ? 'Tailor Your Resume' : 'Synthesize from Profile'}</>}
                                    </button>
                                </div>
                            )}
                        </div>
                    ...
                    {/* Improvement Plan (conditionally visible) */}
                    {showPlan && (
                        <div className="animate-in slide-in-from-top-4 duration-500 bg-[#111118]/80 backdrop-blur-xl border border-amber-500/20 p-12 rounded-[3rem] shadow-2xl space-y-10 mt-12">
                            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                                <div className="space-y-2">
                                    <div className="flex items-center gap-3">
                                        <TrendingUp className="text-amber-500" size={24} />
                                        <h3 className="text-4xl font-black text-white uppercase tracking-tighter">Intelligence Loop <span className="text-amber-500">Recovery</span></h3>
                                    </div>
                                    <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em]">Bridging the {activeApp.analysis?.percentile}% coverage gap for {activeApp.company}</p>
                                </div>
                                <div className="bg-amber-500 text-black px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap">
                                    Target: {improvementPlan?.target_percentile || 95}% Match
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {(improvementPlan?.steps || []).map((step: any, idx: number) => (
                                    <div key={idx} className="bg-white/5 border border-white/5 p-8 rounded-[2.5rem] space-y-6 group hover:border-amber-500/30 transition-all flex flex-col justify-between">
                                        <div className="space-y-6">
                                            <div className="flex items-start justify-between">
                                                <div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center text-amber-500 text-sm font-black">
                                                    0{idx + 1}
                                                </div>
                                                <div className={`px-3 py-1 rounded-full border text-[8px] font-black uppercase tracking-widest ${
                                                    step.type === 'resume_tailoring' ? 'border-blue-500/30 text-blue-500 bg-blue-500/5' :
                                                    step.type === 'skill_acquisition' ? 'border-purple-500/30 text-purple-500 bg-purple-500/5' :
                                            ...
                                            <div>
                                                <h4 className="text-lg font-black text-white uppercase tracking-tight mb-2">{step.title}</h4>
                                                <p className="text-slate-500 text-[10px] leading-relaxed font-medium">{step.description}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                {(!improvementPlan?.steps || improvementPlan.steps.length === 0) && (
                                    <div className="col-span-full py-20 text-center border border-dashed border-white/10 rounded-[2.5rem]">
                                        <p className="text-slate-600 font-black uppercase text-[10px] tracking-widest">Generating Strategy Context...</p>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {viewMode === 'discover' && (
                <div className="animate-in fade-in zoom-in duration-500 py-20 text-center">
                    ...
                    <p className="text-slate-500 font-medium max-w-xl mx-auto mb-8">
                        Launch AI-powered job discovery to find roles that match your profile.
                    </p>
                    <button
                        onClick={() => onNavigate?.('preview')}
                        className="px-10 py-5 bg-blue-600 hover:bg-blue-500 text-white font-black uppercase text-[10px] tracking-[0.3em] rounded-2xl transition-all shadow-xl"
                    >
                        Open Jobs For You
                    </button>
                </div>

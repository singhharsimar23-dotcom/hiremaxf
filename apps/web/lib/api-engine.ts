...
export interface NormalizedIntent {
    ...
    timestamp: string;
    intent_id?: string;
    // Signal-Driven Extensions
    focus_skills?: string[];
    momentum_preference?: 'GROWTH' | 'STABILITY' | 'STRATEGIC';
    strategy_signal?: 'AGGR' | 'DEF';
}
...
export interface JobPointer {
    ...
    salary_raw?: string | null;         // Raw string like "$259k – $389k"
    required_skills?: string[];
    tech_stack?: string[];
    experience_min?: number;
    experience_max?: number;
    work_mode?: string;                 // full_time, contract, remote, etc.
    ...
    posting_age_days?: number | null;   // Days since posted
    // Deterministic signals (attached by hiring-engine before LLM analysis)
    match_score?: number;
    ...
    // Hardened Job Features
    role_category?: string;
    experience_level?: string;
    location_type?: string;
    skill_count?: number;
    skill_rarity_score?: number;
    top_skills?: string[];
...
export const ApiEngine = {
    ...
    async fetchJobPointers(clusterId: string, intent?: NormalizedIntent, signal?: AbortSignal): Promise<JobPointer[]> {
        ...
        if (intent) {
            ...
            if (intent.focus_skills) params.focus_skills = intent.focus_skills.join(',');
            if (intent.momentum_preference) params.momentum_preference = intent.momentum_preference;
            if (intent.strategy_signal) params.strategy_signal = intent.strategy_signal;
        }
        return this.invokeEngine('/job-pointers/by-cluster', 'GET', params, signal);
    },

    /**
     * COMPUTE DETERMINISTIC HIRING DECISION: POST /v1/decision
     */
    async computeHiringDecision(jobId: string, resumeText: string | undefined, targetRole: string, roleTrack: string, signal?: AbortSignal): Promise<any> {
        return this.invokeEngine('/decision', 'POST', {
            job_id: jobId,
            resume_text: resumeText || '', // Allowed to be empty if user is authenticated (backend fetches from context)
            target_role: targetRole,
            role_track: roleTrack
        }, signal, 'hiring-engine');
    },
     ...
     */
    async analyzeMatch(jobId: string, userId: string, jobDescription: string, jobTitle: string, jobCompany: string, deterministicSignals?: any): Promise<{ id: string }> {
        ...
        }, undefined, 'match-analyst').catch(async err => {
            console.warn("[API_ENGINE] Worker trigger failed (may be handled by retry logic):", err);
            // 3. Phantom State Preventer (Vulnerability #11 Fix)
            // If the invocation fails (e.g., 503, network drop), release the UI lock immediately
            await supabase
                .from('match_analysis')
    ...
    async invokeEngine(path: string, method: 'GET' | 'POST', body?: any, signal?: AbortSignal, functionName: string = 'hiring-engine'): Promise<any> {
        ...
        if (!session?.access_token) {
            console.error("[API_ENGINE] Invocation failed: No active session found.");
            throw new Error("AUTHENTICATION_REQUIRED: User must be signed in to perform this action.");
        }

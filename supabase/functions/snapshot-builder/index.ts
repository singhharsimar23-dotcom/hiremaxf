...
serve(async (req) => {
    ...
    try {
        ...
        // 1. IDENTITY VERIFICATION (SEC-004 - Service Role Aware)
        ...
        if (isServiceRole) {
            // TRUSTED PATH: System invocation
            if (!user_id) throw new Error("Service Role invocation requires explicit user_id in body")
            console.log(`[AUTH] Snapshot Builder invoked by Service Role for User ${user_id}`)
        } else {
            // USER PATH: Validate JWT and match Identity
        ...
        const [
            { data: work }, { data: projects }, { data: skills },
            { data: edu }, { data: achievements }, { data: pubs }, { data: oss }, { data: profile }
        ] = await Promise.all([
        ...
        let weightSet = weightSets?.find((ws: any) => ws.segment_criteria?.role_category === roleCategory);

        // Fallback to global if no specific segment found
        if (!weightSet) {
            weightSet = weightSets?.find((ws: any) => !ws.segment_criteria || Object.keys(ws.segment_criteria).length === 0);
        }
        ...
        let completeness = 0;
        if (work?.length) completeness += (weights.work_experience_weight || 10);
        if (edu?.length) completeness += (weights.education_weight || 5);
        if (skills?.length && skills.length >= 5) completeness += (weights.skills_weight || 10);
        if (projects?.length) completeness += (weights.projects_weight || 5);
        ...
        const lastSig = signals.sort((a: any, b: any) => new Date(b.ingested_at).getTime() - new Date(a.ingested_at).getTime())[0];

...
serve(async (req) => {
    ...
    try {
        ...
        const { user_id, job_id, job_description, job_title, job_company } = body;
        if (!user_id || !job_id) {
        ...
        const profileSummary = [
            ...
            `Experience: ${(profileData.experience || []).map((e) => `${e.title} at ${e.organization}`).join('; ') || 'None listed'}`,
            `Projects: ${(profileData.projects || []).map((p) => p.name).join(', ') || 'None listed'}`,
            `Education: ${(profileData.education || []).map((e) => `${e.degree} from ${e.institution}`).join('; ') || 'Not specified'}`,
        ].join('\n');
        const jobSummary = [

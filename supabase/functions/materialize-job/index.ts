...
serve(async (req: Request) => {
    ...
    try {
        ...
        if (pError || !pointer) throw new Error(`POINTER_NOT_FOUND: ${job_id}`);
        ...
        if (!company) throw new Error("COMPANY_DATA_MISSING");
        ...
        if (pointer.source_type === 'GREENHOUSE') {
            ...
            if (!token) throw new Error("PARTNER_TOKEN_MISSING: GREENHOUSE");
            ...
            if (!token) throw new Error("PARTNER_TOKEN_MISSING: LEVER");
        ...
        if (rejectedPatterns.some((p: string) => title.includes(p)) || description.length < 200) {
        ...
        if (closureIndicators.some((i: string) => rawBody.toLowerCase().includes(i))) {
        ...
        const job_id_fallback = (body as any)?.job_id;

...
serve(async (req) => {
    ...
    try {
        ...
        const { data: pendingApps, error: sweepError } = await supabase
            ...
            .limit(500);
        if (sweepError)
            console.error(`[ORCHESTRATOR] Sweep Query Error:`, sweepError);
        if (pendingApps && pendingApps.length > 0) {
            console.log(`[ORCHESTRATOR] Sweeping ${pendingApps.length} pending applications for Kill Zone analysis...`);
            const results = await Promise.allSettled(pendingApps.map(app => supabase.functions.invoke('execution-engine', {

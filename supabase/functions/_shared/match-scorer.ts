...
export class MatchScorer {
     ...
     */
    static calculateScore(input: MatchInput): number {
        // 1. Skill Overlap (40%)
        ...
        if (!input.location_match) return Math.min(total, 0.4); // Hard cap on remote/onsite mismatch
        if (skillScore < 0.3) return total * 0.5; // Penalty for low skill overlap
     ...
     */
    static isActionable(score: number, threshold: number = 0.75): boolean {

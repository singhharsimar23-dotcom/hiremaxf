...
export type RoleCategory =
    ...
    | 'product' | 'design' | 'systems' | 'embedded' | 'blockchain' | 'tpm' | 'other';

/**
 * Technical Role Categories for filtering
 */
export const TECH_CATEGORIES: RoleCategory[] = [
    'backend', 'frontend', 'fullstack', 'mobile',
    'devops', 'sre', 'ml', 'data', 'security',
    'product', 'systems', 'embedded', 'blockchain', 'tpm'
];
...
/**
 * Filter for tech-only roles as per forensic audit requirements
 */
export function isTechJob(title: string): boolean {
    const t = title.toLowerCase();
    const techPatterns = [
        /\b(software enginee|swe|developer|engineer|fullstack|full-stack|backend|frontend|front-end)\b/,
        /\b(devops|sre|site reliability|infrastructure|platform|cloud|aws|gcp|azure)\b/,
        /\b(security|cybersecurity|infosec|appsec|penetration)\b/,
        /\b(machine learning|ml|ai|deep learning|nlp|computer vision|data enginee|data science)\b/,
        /\b(product manager|product lead|pm)\b/,
        /\b(systems enginee|embedded)\b/,
        /\b(blockchain|web3|ethereum|solidity)\b/,
        /\b(technical program manager|tpm)\b/
    ];
    return techPatterns.some(pattern => pattern.test(t));
}

...
export const SIGNAL_HALF_LIVES: Record<string, number | null> = {
...
export const DECAY_FLOORS: Record<string, number> = {
    'EXPERIENCE': 0.60,
    'EDUCATION': 0.70,
    'PAST_JOB_TITLE': 0.50,
    'DEGREE': 0.80,
...
export const SOURCE_AUTHORITY_SCORES: Record<string, number> = {
 ...
 */
export function calculateTemporalDecay(type: string, timestamp: string | Date): number {
...
export function calculateVerificationStrength(params: {
    ...
    sourceCount?: number;
    hasConsistentTimeline?: boolean;
    hasEndorsement?: boolean;
    endorsementQuality?: number;
    hasArtifact?: boolean;
    artifactType?: 'CODE' | 'DOCUMENT' | 'SCREENSHOT';
}): number {
    let score = 0.50; // Base

    if (params.hasUrlProof) score += 0.30;

    if (params.hasMultipleSources && params.sourceCount) {
        score += Math.min(params.sourceCount * 0.05, 0.20);
    }

    if (params.hasConsistentTimeline) score += 0.10;
    ...
    if (params.hasArtifact) {
        if (params.artifactType === 'CODE') score += 0.25;
        else if (params.artifactType === 'DOCUMENT') score += 0.20;
        else if (params.artifactType === 'SCREENSHOT') score += 0.10;
    }
 ...
 */
export function calculateCrossVerificationBoost(sources: string[]): number {
    const uniqueSources = new Set(sources);
    const diversity = uniqueSources.size;

    if (diversity <= 1) return 1.00;
...
export function calculateFinalWeight(signal: {
    ...
    cross_verification_boost: number;
    final_weight: number;
} {
    ...
    const final_weight = Math.min(
        1.0,
        base_weight * temporal_decay * source_authority * verification_strength * cross_verification_boost
    );
 ...
 * Skill Level Helper
 */
export function getSkillLevel(score: number): 'NOVICE' | 'BEGINNER' | 'INTERMEDIATE' | 'ADVANCED' | 'EXPERT' {
    if (score >= 0.86) return 'EXPERT';
    if (score >= 0.71) return 'ADVANCED';
    if (score >= 0.51) return 'INTERMEDIATE';
    if (score >= 0.31) return 'BEGINNER';
    return 'NOVICE';
}

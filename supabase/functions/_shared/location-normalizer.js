export var USLocation;
(function (USLocation) {
    USLocation["WEST"] = "US-WEST";
    USLocation["EAST"] = "US-EAST";
    USLocation["CENTRAL"] = "US-CENTRAL";
    USLocation["REMOTE"] = "US-REMOTE";
    USLocation["OTHER"] = "US-OTHER";
    USLocation["REJECTED"] = "REJECTED";
...
export class LocationNormalizer {
    static normalize(raw) {
        ...
        // State Abbreviations
        if (/\b(ca|wa|or|nv|az|id|mt|wy|ut|co|nm|ak|hi)\b/.test(loc))
            return USLocation.WEST;
        if (/\b(ny|ma|pa|nj|ct|ri|vt|nh|me|de|md|va|wv|nc|sc|fl)\b/.test(loc))
            return USLocation.EAST;
        if (/\b(tx|il|mi|OH|ga|nc|tn|mo|mn|wi|in|ky|al|la|ok|ks|ar|ia|ms|ne|sd|nd)\b/.test(loc))
            return USLocation.CENTRAL;
        // General Fallbacks
        if (loc.includes("united states") || loc.includes("usa") || loc.includes(" us "))
            return USLocation.OTHER;
        return USLocation.REJECTED;
    }
    static isAccepted(loc) {

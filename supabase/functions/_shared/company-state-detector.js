...
(function (CompanyState) {
    CompanyState["DORMANT"] = "dormant";
    CompanyState["STABLE"] = "stable";
    CompanyState["GROWTH"] = "growth";
    CompanyState["HYPERGROWTH"] = "hypergrowth";
    CompanyState["ACQUISITION_MODE"] = "acquisition";
})(CompanyState || (CompanyState = {}));
export class CompanyStateDetector {
    /**
     * Deterministic state classification with Sector and Stage awareness.
     */
    static detectState(signals) {
        // High priority: Negative signals (Absolute blockers)
        if (signals.layoff_count_90d > 0)
            return CompanyState.DORMANT;
        if (signals.acquisition_rumors)
            return CompanyState.ACQUISITION_MODE;
        ...
        // Growth signals
        if (signals.days_since_funding < 90 &&
        ...
        if (['Series D', 'Series E', 'IPO'].includes(signals.stage || '')) {
            if (signals.employee_growth_rate_90d > 0.08)

...
export class AdzunaService {
    static async fetchJobs(query, location, limit = 20, debug = false) {
        ...
        try {
            ...
            if (debug) {
                return await Promise.all(results.map(async (job) => {
            ...
            return await Promise.all(results.map(async (job) => {
                ...
                const fingerprint = await generateFingerprint(job.company?.display_name || "Unknown Company", norm.title, norm.location_raw);
                return {

...
export class RemoteOKService {
    static async fetchJobs(query = "software monitor", debug = false) {
        ...
        try {
            ...
            if (debug) {
                return await Promise.all(actualJobs.slice(0, 5).map(async (job) => {
            ...
            return await Promise.all(actualJobs.map(async (job) => {
                ...
                const fingerprint = await generateFingerprint(job.company, norm.title, norm.location_raw);
                return {
                    // id: fingerprint,

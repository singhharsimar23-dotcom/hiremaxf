...
function normalizeRole(title: string): string {
    ...
    if (/\b(frontend|front-end|react|vue|angular)\b/.test(t)) return 'frontend';
    if (/\b(fullstack|full-stack|full stack)\b/.test(t)) return 'fullstack';
    if (/\b(mobile|ios|android|flutter)\b/.test(t)) return 'mobile';
    if (/\b(devops|sre|infrastructure|platform|cloud)\b/.test(t)) return 'devops';
    if (/\b(machine learning|ml|ai|deep learning|data scientist)\b/.test(t)) return 'ml';
    if (/\b(data engineer|analytics|etl)\b/.test(t)) return 'data';
    if (/\b(security|infosec|appsec)\b/.test(t)) return 'security';
    if (/\b(product manager|pm)\b/.test(t)) return 'product';
...
function normalizeSeniority(title: string): string {
    ...
    if (/\b(junior|jr|entry|new grad)\b/.test(t)) return 'junior';
    ...
    if (/\b(manager|director|vp)\b/.test(t)) return 'manager';
...
function normalizeLocation(location: string): string {
    ...
    if (/\b(remote|anywhere|worldwide|wfh)\b/.test(l)) return 'remote';
    if (/\b(hybrid|flexible)\b/.test(l)) return 'hybrid';
    return 'onsite';
}

function calculateQualityScore(job: { title?: string; company?: string }): number {
    let score = 0;
    if (job.title && job.title.length > 5) score += 0.5;
    if (job.company && job.company.length > 1) score += 0.5;
...
async function fetchCordJobs(): Promise<any[]> {
    try {
...
async function fetchHiredJobs(): Promise<any[]> {
    try {
...
async function fetchRemoteCoJobs(): Promise<any[]> {
    try {
...
async function fetchJobspressoJobs(): Promise<any[]> {
    try {
...
async function fetchCrunchbaseJobs(): Promise<any[]> {
    try {
...
async function fetchJoobleJobs(apiKey: string): Promise<any[]> {
    if (!apiKey) {
...
async function fetchCareerjetJobs(apiKey: string): Promise<any[]> {
    if (!apiKey) {
    ...
    try {
        ...
        const response = await fetchWithTimeout(
            `http://public.api.careerjet.net/search?locale_code=en_US&keywords=software+engineer&affid=${apiKey}`
        );
        if (!response?.ok) return [];
        const data = await response.json();
        const jobs = data.jobs || [];
...
serve(async (req: Request) => {
    ...
    try {
        ...
        for (const job of allJobs) {
            ...
            } catch (e: any) { console.error('[TECH_SCRAPER] Job error:', e.message); }
        }

    ...
    try {
        ...
        if (subPath === '/job-pointers/by-cluster' && req.method === 'GET') {
            ...
            if (pError)
                throw pError;
            // 4. DATA MAPPING
            const jobIds = (pointers || []).map((p) => p.company_id).filter(id => id);
            let companyMap = {};
            if (jobIds.length > 0) {
            ...
            const result = (pointers || []).map((p) => {
                // Heuristic Location Labeling
                let displayLocation = p.location_name || p.location_type || 'US-REMOTE';
                if (!p.location_name) {
                    if (p.source_url?.toLowerCase().includes('remote'))
                        displayLocation = 'Remote (Verified)';
                    else if (p.location_type === 'onsite')
                        displayLocation = 'Onsite (Unknown Location)';
                }
                return {
                    job_id: p.id,

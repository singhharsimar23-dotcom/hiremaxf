...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { source, source_type, action, payload, url_classification, command_id } = body
        ...
        if (source_type === 'URL') {
            ...
            if (!payload?.url) throw new Error("URL sources MUST receive payload.url");
        }
        ...
        // 5. REGISTER OR ADOPT COMMAND (The clean Reconstruction)
        let command;
        if (command_id) {
            // ADOPTION PATH: Frontend used RPC to create command. We must link it.
            const { data: existingCmd, error: fetchErr } = await supabaseClient
                .from('ingestion_commands')
                .select()
                .eq('id', command_id)
                .single();

            if (fetchErr || !existingCmd) {
                throw new Error(`Invalid command_id: ${command_id}. RPC Handover failed.`);
            }
            command = existingCmd;

            // Link Session & Update Status (Service Role has permission)
            ...
            // LEGACY PATH: Upsert based on idempotency_key
            const { data: newCmd, error: cmdError } = await supabaseClient
                .from('ingestion_commands')
                .upsert({
                    user_id,
                    source,
        ...
        if (source_type === 'URL') workerName = 'worker-external'
        if (source_type === 'FILE') workerName = 'worker-resume'
        ...
        try {
            // SERVICE HANDOVER: Use Service Role to invoke worker
            // This decouples the worker from the user's session (prevents expiry issues)
            ...
            await supabaseClient.functions.invoke(workerName, {
                ...
                body: {
                    ...
                    action,
                    payload,
                    ...payload // FIX: Spread payload for top-level access (url, file_path)
                }
            })
        } catch (workerErr: any) {

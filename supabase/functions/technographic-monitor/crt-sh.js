...
export class CrtShService {
    static async checkDomains(domains, debug = false) {
        const pointers = [];
        for (const domain of domains) {
            try {
                if (debug)
                    console.log(`[Crt.sh] Checking ${domain}...`);
                const subdomains = await this.fetchSubdomains(domain);
                const careerSubdomains = subdomains.filter(s => /^(career|jobs|work|join|hiring|scout|talent)\./i.test(s));
                if (careerSubdomains.length > 0) {
                    if (debug)
                        console.log(`[Crt.sh] Found career subdomains for ${domain}:`, careerSubdomains);
                    for (const sub of careerSubdomains) {
                        const fingerprint = await generateFingerprint(domain.split('.')[0], // Company name approx
                        "Technographic Signal: " + sub, "Remote");
                        pointers.push({
    ...
    static async fetchSubdomains(domain) {
        ...
        try {
            ...
            if (e.name === 'AbortError')
                console.error(`[Crt.sh] Timeout on ${domain}`);
            return [];
        }
    }
}

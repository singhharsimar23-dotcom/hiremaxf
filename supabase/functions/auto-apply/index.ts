...
serve(async (req) => {
    ...
    try {
        ...
        if (!user_id || !application_id) throw new Error("MISSING_PARAMS");
        ...
        if (!snapshot) throw new Error("PROFILE_SNAPSHOT_NOT_FOUND: You must build your profile before applying.");

...
export class GitHubWatcher {
    static async scan(targets, debug = false) {
        const jobs = [];
        await Promise.all(targets.map(async (target) => {
            try {
                ...
                if (!content)
                    content = await this.fetchReadme(target, "master");
                if (!content)
                    content = await this.fetchReadme(target, "canary"); // Vercel/Next.js
                if (!content)
                    content = await this.fetchReadme(target, "develop");
                if (!content) {
                ...
                if (intent) {
                    const fingerprint = await generateFingerprint(target.split("/")[0], // owner
    ...
    static async fetchReadme(repo, branch) {

...
serve(async (req) => {
    ...
    try {
        ...
        return Guardrails.handleError(supabaseClient, error, "DISCOVERY_WORKER", { company_id: req.company_id });
    }
});
async function logEvent(supabase, companyId, mode, attempted, success, count, reason, startTime) {
    const discoveryDay = new Date().toISOString().split('T')[0];

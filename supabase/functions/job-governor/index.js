...
serve(async (req) => {
    ...
    try {
        ...
        if (stateError || !state)
            throw new Error("GOVERNOR_STATE_MISSING");
        ...
        const successCount = events?.filter((e) => e.success && e.pointers_created > 0).length || 0;
        ...
        if (redCount >= 3)
            nextMode = 'READ_ONLY';
        else if (blindnessFlag || redCount >= 1)
            nextMode = 'SAFE';
        else if (successCount > 0 && scrapeSuccessRate < 0.85)
            nextMode = 'CONTROLLED';

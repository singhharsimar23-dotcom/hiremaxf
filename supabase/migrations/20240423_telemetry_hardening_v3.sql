...
-- 2. Cleanup existing duplicate metrics to allow UNIQUE constraint
DO $$
BEGIN
    -- Merge duplicates into temporary table
    CREATE TEMP TABLE merged_metrics AS
    SELECT 
        ts, 
        source, 
        ...
        SUM(skipped_dupe) as skipped_dupe, 
        SUM(skipped_low_quality) as skipped_low_quality, 
        SUM(duration_ms) as duration_ms,
        MAX(run_id::text)::uuid as run_id,
        MIN(created_at) as created_at
    FROM public.ingestion_metrics
    GROUP BY ts, source;

    -- Clear original table
    TRUNCATE public.ingestion_metrics;

    -- Re-insert merged data
    INSERT INTO public.ingestion_metrics (
        ts, source, attempted, ingested, failed, skipped_dupe, skipped_low_quality, duration_ms, run_id, created_at
    )
    SELECT ts, source, attempted, ingested, failed, skipped_dupe, skipped_low_quality, duration_ms, run_id, created_at
    FROM merged_metrics;

    DROP TABLE merged_metrics;
END $$;

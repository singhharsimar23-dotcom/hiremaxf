...
export const RebuildStandaloneView: React.FC<RebuildStandaloneViewProps> = ({ plan, credits, setCredits, onRebuildSuccess, onUpgrade, history, preFilledContext, activeJobs, dispatchJob }) => {
  ...
  const runningJobId = useMemo(() => {
    return Object.keys(activeJobs).find(id => activeJobs[id].type === 'REBUILD' && activeJobs[id].status === 'RUNNING');
  }, [activeJobs]);
  ...
  useEffect(() => {
    if (runningJobId) {
      ...
      if (job.status === 'COMPLETED' && job.result) {
        setRebuiltResume(job.result.newResume);
        setFinalMeta({ role: job.payload.role, label: `${job.payload.role} @ ${job.payload.roleTrack}` });
        setStep('result');
  ...
  useEffect(() => {
    if (preFilledContext) {
      ...
      if (preFilledContext.text && preFilledContext.role && !runningJobId) {
  ...
  const startRebuild = async (overrideText?: string, overrideRole?: string, overrideTrack?: RoleTrack, overrideGate?: 'SAFE' | 'BORDERLINE' | 'LOCKED') => {
    ...
    await dispatchJob('REBUILD', { prompt, role: roleToUse, roleTrack: trackToUse });
  };

  const handleCommit = () => {
    if (rebuiltResume) {
      if (!hasUnlimited && !preFilledContext) setCredits(credits - 1);
      onRebuildSuccess(rebuiltResume, "", finalMeta.label);
    }
  };
    ...
    return (
      <div className="max-w-3xl mx-auto py-24 px-10 text-center animate-in fade-in duration-500">
        ...
        <p className="text-slate-400 text-lg font-medium leading-relaxed mb-12">
          High-fidelity rebuilding has reached its operational limit. To finalize your document with priority processing, authenticate with a private API key.
        </p>
        <div className="space-y-6">
          <button 
